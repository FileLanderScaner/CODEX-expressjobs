...
| RLS real smoke | `AUTH_FAILURE` | `.env.rls` is present; repeated smoke attempts fail on first client login with invalid credentials before RLS policy execution. |
